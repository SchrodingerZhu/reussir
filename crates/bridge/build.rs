fn main() {
    ...
    // Check if REUSSIR_BRIDGE_LIBRARY_DIR is provided
    if let Ok(bridge_lib_dir) = std::env::var("REUSSIR_BRIDGE_LIBRARY_DIR") {
        // Use existing bridge library
        println!("cargo:rustc-link-search=native={}", bridge_lib_dir);
        println!("cargo:rustc-link-lib=dylib=MLIRReussirBridge");
    } else {
        // Build bridge library with cmake
