...
use crate::literal::{unescape_char, unescape_string};
...
impl Emitter<'_> {
    ...
    /// `import p (as n)?;` encodes as the bound name and the target path: the
    /// renaming identifier when `as` is present, otherwise the path's last
    /// segment.
